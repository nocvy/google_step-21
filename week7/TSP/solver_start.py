...
def solve(cities):
    ...
    min_tour = g_tour
    min_path =total_path_length(min_tour, dist, N)

    # スタート位置を変えてスコアを出し、スコアが最も小さいものを保存する
    # N <= 512 の時 -> 全ての頂点について調べる
    # 512 < N <= 2048 の時 -> 10個の頂点をランダムに指定し調べる
    # 2048 < N の時 -> 3個の頂点をランダムに指定し調べる
    
    if N <= 512: # 全ての頂点について調べる
        for i in range(1,N):
            g_tour = greedy(cities, N, dist, i)
            t_tour = opt_2(g_tour, dist)
            cur_path = total_path_length(t_tour, dist, N)

            if min_path > cur_path:
                min_tour = t_tour
                min_path = cur_path

    elif N <= 2048: # 10個の頂点をランダムに指定し調べる
        for i in range(10):
            current_city = random.randint(0, len(cities)-1) # スタート位置決定

            g_tour = greedy(cities, N, dist, current_city)
            t_tour = opt_2(g_tour, dist)
            cur_path = total_path_length(t_tour, dist, N)
    ...
    else: # 3個の頂点をランダムに指定し調べる
        for i in range(3):
            current_city = random.randint(1, len(cities)-1) # スタート位置決定
    ...
    m_tour = move_subsequence(min_tour, dist, 7)
...
def greedy(cities, N, dist, start):
    current_city = start
    unvisited_cities = set(range(0, N)) #(1, N)だったのをスタート地点を変えられるように拡張
